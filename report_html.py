    ...
    try:
        ...
        try:
            ...
            if all(v is not None for v in (teto, poc_v, piso, alvo_a, alvo_b)):
                # A — range/equilíbrio
                # ordena as 3 linhas pelo VALOR (o POC pode estar acima do teto ou
                # abaixo da defesa — o desenho tem que respeitar a ordem real)
                trio = sorted([(teto, WL, f"teto {_fmt(teto)}", ""),
                               (poc_v, PC, f"{pivo_lbl} {_fmt(poc_v)}", "5,4"),
                               (piso, BD, f"defesa {_fmt(piso)}", "5,4")],
                              key=lambda t: -t[0])
                ys_a = [55, 125, 195]
                poc_y = ys_a[[i for i, t in enumerate(trio) if t[1] == PC][0]]
                ima_txt = (f"o {pivo_lbl} é o ímã do dia" if poc_y == 125 else
                           (f"{pivo_lbl} acima do teto: ímã no topo do range" if poc_y == 55 else
                            f"{pivo_lbl} abaixo da defesa: ímã no fundo do range"))
                a = ("".join(_ln(y, c, l, d) for y, (v, c, l, d) in zip(ys_a, trio))
                     ...
                     + _txt(300, (118 if poc_y == 125 else (75 if poc_y == 55 else 178)), ima_txt, DM, 10))
                ...
                # B — rompimento comprador (POC entre teto e alvo quando POC > teto)
                poc_y_b = 215 if poc_v <= teto else (90 if poc_v <= alvo_a else 25)
                b = (_ln(40, BD, f"alvo {_fmt(alvo_a)}", "5,4") + _ln(140, WL, f"teto {_fmt(teto)}")
                     + _ln(poc_y_b, PC, f"{pivo_lbl} {_fmt(poc_v)}", "5,4")
                ...
                # C — perda do suporte (POC entre defesa e alvo quando POC < defesa)
                poc_y_c = 40 if poc_v >= piso else (160 if poc_v >= alvo_b else 228)
                c = (_ln(poc_y_c, PC, f"{pivo_lbl} {_fmt(poc_v)}", "5,4") + _ln(115, WL, f"defesa {_fmt(piso)}")
...
<aside class="sidebar" id="sidebar">
  ...
  <nav class="nav" id="nav">
    ...
    <a href="estatistica.html" target="_blank"><span class="ic">&#127919;</span> Estatística de acertos</a>
